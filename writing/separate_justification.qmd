...
$$
\begin{bmatrix}
\hat{y}^{*(1)} \sim \mathcal{N}(\beta_1^{(1)}\cdot x_{new} + \beta_0^{(1)}, (\sigma^2)^{(1)}) \\
\vdots \\
\hat{y}^{*(2000)} \sim \mathcal{N}(\beta_1^{(2000)}\cdot x_{new} + \beta_0^{(2000)}, (\sigma^2)^{(2000)})
\end{bmatrix}
,
\begin{bmatrix}
\hat{p}^{(1)} \sim Bern\bigg(\frac{1}{1 + e^{-(\gamma_1^{(1)}\cdot x_{new} + \gamma_0^{(1)})}}\bigg) \\
\vdots \\
\hat{p}^{(2000)} \sim Bern\bigg(\frac{1}{1 + e^{-(\gamma_1^{(2000)}\cdot x_{new} + \gamma_0^{(2000)})}}\bigg)
\end{bmatrix}
$$


And $(\hat{y}^{*(1)}, ..., \hat{y}^{*(20000)})$ would be our posterior predictive distribution under our model for $y^*$, while $(\hat{p}^{(1)}, ..., \hat{p}^{(20000)})$ would be our posterior predictive distribution under our model for $p$.

But remember that our response in this modeling schema is the product of the outputs from these two models. And so we want a the posterior predictive distribution of $y_{new} = \hat{y}^*\hat{p}$, but it's unclear how we should combine the predictive distributions from the individual models to get here. We might just match MCMC iteration i from each model together, but what makes this matching more correct than shuffling the iterations and then matching them up? To understand what to do about this, we'll dive into some theory behind building the models simultaneously.
...
But now, if we look at this closely we can see that what we really have here is a full separation of the posteriors for the individual models for $p$ and $y^*$ as seen in our derivation in the \*\*separate fit\* section. This means that we can write:
